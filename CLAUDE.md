...
- Code quality (2026-06-13, merged #52–#60): test coverage 36→109 incl. Firestore rules emulator suite in CI; 5 send routes collapsed onto `EMAIL_TYPES`; event-detail page split into `useEvent`/`useBulkSend` + `lib/csv`/`lib/bulk-send`/`lib/contacts`; add-guest fields relabeled Name/Email with smart paste. Still open: extract presentational subcomponents from `events/[eventId]/page.jsx` (~900 lines, mostly JSX now); finish #19 brand de-hardcoding (`ORGANIZER_CLAIM` centralized server-side, but the string is still inline in `firestore.rules` + `Header`).
- Local dev: `vercel env pull` mangles `FIREBASE_SERVICE_ACCOUNT` (turns the private key's `\n` into real newlines → invalid JSON). Set `FIREBASE_SERVICE_ACCOUNT_B64` (base64 of the JSON) in `.env.local` instead — `firebase-admin.js` prefers it.
