...
import { buildCustomEmail, buildCustomPlainText } from '@/lib/email-templates'
...
export async function POST(request) {
  ...
  const { eventId, subject, body, token } = await request.json().catch(() => ({}))
  if (!eventId || !subject || !body || !token) {
    return NextResponse.json({ error: 'Missing eventId, subject, body, or token' }, { status: 400 })
  }
  ...
  const emailRes = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: invite.email,
      subject,
      html: buildCustomEmail({ invite, body, link }),
      text: buildCustomPlainText({ invite, body, link }),
      headers: {
